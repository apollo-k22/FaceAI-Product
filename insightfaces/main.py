...
def append_failed_json_data(data, path):
    _, token = generate_token()
    facedata = {
        'image_path': path,
        'face_token': str(token),
...
def append_empty_json_data(data):
    data['faces'].append([])
    data['results'].append([])
...
def recognition(path1, paths2):
    ...
    if bboxes1.shape[0]==0:
        jsondata = append_empty_json_data(jsondata)
        jsondata = build_json_data(jsondata, 0)
    ...
    for index, path2 in enumerate(paths2):
        ...
        if bboxes2.shape[0]==0:
            jsondata = append_failed_json_data(jsondata, path2.__str__())
        ...
        res = get_similarity_str(jsondata, sim[index], path2.__str__())
